...
const deletePropertyController = async (req, res, next) => {
  try {
    ...
    if (!id)
      return next(
        new CustomError("id is required to delete the property", 400)
      );

    const deleteProperty = await Property.findByIdAndDelete(id);
    if (!deleteProperty)
      return next(new CustomError("Error in deleting the property", 400));
...
const updatePropertyController = async (req, res, next) => {
  try {
    ...
    if (!id)
      return next(
        new CustomError("id is required to update the property", 400)
      );
    ...
    if (!updatedProperty)
      return next(new CustomError("Error in updating the property", 400));
...
const viewPropertyController = async (req, res, next) => {
  try {
    const { id } = req.params;
    if (!id)
      return next(new CustomError("id is required to veiw property", 401));

    const propertyDetails = await Property.findById(id);

    if (!propertyDetails)
      return next(new CustomError("Error in fetching property details", 400));
...
const viewMyPropertyController = async (req, res, next) => {
  try {
    const id = req.user._id;
    if (!id) return next(new CustomError("my peroperty id not fouund", 400));

    const property = await Property.find({ host: id });
    if (!property) return next(new CustomError("No property found", 400));

    res.status(200).json({
      message: "My peroperties",
      data: property,
    });
  } catch (error) {
    next(new CustomError(error.message), 500);
